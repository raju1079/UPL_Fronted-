...
import { fetchProgramById, fetchProgramId, fetchProgramList, getProgramCourseCombo, registerSubsciber } from '../redux/actions/Actions';
...
const HeroPromotionForm = () => {
  ...
  const [item, setItem] = useState([]);
    ...
    const handleCourseChange = (e) =>{
      //console.log('course is',e.target.value)
      setCourseId(e.target.value)
    }
    //console.log('program code is',programId)
   // console.log('course code is',courseId)
                            ...
                            <Grid container  spacing={2} sx={{border: '1px solid rgba(0,0,0,0.125', zIndex:"3"}}>
                                      ...
                                      >
                                          {
                                              fetchPrograms.map((eachItem,i)=>(
                                                  <MenuItem key={eachItem.program_id} value={eachItem.program_id}>{eachItem.program_name}</MenuItem>
                                              ))
                                          }
                                      </Select>
                                  </FormControl>        
                                ...
                                <FormControl fullWidth>
                                      ...
                                      >
                                          {
                                              item?.map((eachItem,i)=>(
                                                  <MenuItem key={eachItem.course_id} value={eachItem.course_id}>{eachItem.course_name}</MenuItem>
                                              ))
                                          }
                                      </Select>
                                  </FormControl>        
